...
def bktOrderFunc(side,qty,limit_price,take_profit_price,stop_loss_price):
    ...
    for accountnumber in accounts:
        if accountnumber == "U1XXXXXX5": 
            #IRA account
            qty = 15 
            contract = Future(symbol = "MES", lastTradeDateOrContractMonth = latest_exp_month, exchange = "CME")
        else: 
            #Regular account
            qty = 6 
            contract = Future(symbol = "ES", lastTradeDateOrContractMonth = latest_exp_month, exchange = "CME")
...
def cancel_bracket_orders_and_close_position():
    ...
    if len(pos_df) > 0:
        positions_all = pos_df[pos_df['position'] != 0]
        for accountnumber in accounts:
            print(accountnumber)
            try:
                positions = positions_all[positions_all['account'] == accountnumber]
